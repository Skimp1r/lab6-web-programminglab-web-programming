(function () {
  ...
  function sendRuntimeMessage(payload) {
    return new Promise((resolve) => {
      try {
        chrome.runtime.sendMessage(payload, (resp) => resolve(resp || null));
      } catch (_) {
        resolve(null);
      }
    });
  ...
  async function loadActivityStats() {
    try {
      ...
      const rawUrl = active && active.url ? String(active.url) : '';
      const host = /^https?:\/\//i.test(rawUrl) ? new URL(rawUrl).hostname : '';
      state.currentHost = host;

      if (host) {
        const siteResp = await sendRuntimeMessage({ type: 'get-site-stats', host });
        state.currentSiteStat = siteResp && siteResp.ok ? siteResp.stat : null;
      } else {
        state.currentSiteStat = null;
      }

      const topResp = await sendRuntimeMessage({ type: 'get-top-sites', limit: 5 });
      state.topSites = topResp && topResp.ok && Array.isArray(topResp.items) ? topResp.items : [];
    } catch (_) {
      state.currentSiteStat = null;
      state.topSites = [];
    }
  ...
  function renderSiteStatsHtml() {
    const current = state.currentSiteStat
      ? `<div><strong>${escapeHtml(state.currentHost)}</strong>: ${Number(state.currentSiteStat.totalMinutes || 0).toFixed(
          2
        )} мин, визитов: ${state.currentSiteStat.visits || 0}</div>`
      : `<div class="help">${
          state.currentHost ? `Для ${escapeHtml(state.currentHost)} статистики пока нет.` : 'Открой обычный сайт (http/https).'
        }</div>`;

    const top =
      state.topSites && state.topSites.length
        ? `<ol class="cmp-list">${state.topSites
            .map(
              (s) =>
                `<li>${escapeHtml(s.host)} — ${Number(s.totalMinutes || 0).toFixed(2)} мин (визитов: ${s.visits || 0})</li>`
            )
            .join('')}</ol>`
        : '<div class="help">Топ сайтов пока пуст.</div>';

    return `<div class="site-stats">${current}<div style="margin-top:8px"><strong>Топ активных сайтов:</strong>${top}</div></div>`;
  }
